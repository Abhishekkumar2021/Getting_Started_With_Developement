...
func WeightController(w http.ResponseWriter, r *http.Request) {
	...
	convertedWeight, err := utils.ConvertWeight(q.Get("from_unit"), q.Get("to_unit"), value)
	if err != nil {
		// Send error response
		errorMap := map[string]string{
			"message": "Something went wrong!",
...
func TemperatureController(w http.ResponseWriter, r *http.Request) {
	...
	convertedTemperature, err := utils.ConvertTemperature(q.Get("from_unit"), q.Get("to_unit"), value)
	if err != nil {
		// Send error response
		errorMap := map[string]string{
			"message": "Something went wrong!",
